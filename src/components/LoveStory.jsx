...
const LoveStory = () => {
  ...
  return (
    <div className="relative py-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-blue-200 to-white text-gray-700">
      ...
      <motion.img src={`${cdnBaseUrl}leaf-branches-3.webp`} alt="Ornamen" className="blur-[1px] absolute top-[370px] left-[-43px] w-[190px] h-auto z-0 rotate-[165deg] opacity-80" onError={(e) => e.target.style.display = 'none'} variants={leaf3Variants} initial="hidden" whileInView="visible" viewport={{ once: true }} />
      <motion.img src={`${cdnBaseUrl}leaf-branches-1 tuned.webp`} alt="Ornamen" className="blur-[1px] absolute bottom-[160px] right-[-10px] w-[125px] h-auto z-0 opacity-80" onError={(e) => e.target.style.display = 'none'} variants={leaf1Variants} initial="hidden" whileInView="visible" viewport={{ once: true }} />
      <img src={`${cdnBaseUrl}floral-bouquet-2.webp`} alt="Ornamen" className="blur-[1px] absolute top-[175px] right-[20px] w-[200px] h-auto z-0 opacity-70" onError={(e) => e.target.style.display = 'none'} />
          ...
          return (
            ...
            >
              <div className={`w-full pl-12 md:w-1/2 ${isLeftOnDesktop ? 'md:pr-12 md:pl-0' : 'md:pl-12'}`}>
                ...
                <div className="flex flex-col rounded-lg overflow-hidden shadow-lg transform hover:scale-[1.02] transition-transform duration-300 bg-white/40 backdrop-blur-sm">
                  <div className="w-full h-48 md:h-64">
                    <img 
                      src={story.image || `${cdnBaseUrl}gallery-1.webp`}
                      alt={story.title}
                      className="w-full h-full object-cover"
                    />
                  </div>
                  
                  <div className="w-full p-5 flex items-center justify-center border-t border-white/20">
                    <p className="font-sans text-[15px] leading-relaxed text-center drop-shadow-sm">
                      {story.description}
                    </p>
                  </div>
                </div>
